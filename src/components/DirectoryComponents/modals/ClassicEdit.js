import React, {useContext, useState} from 'react';
import {Button, Modal} from "react-bootstrap";

import {Context} from "../../../index";
import {
    createClassicModel,
    getClassicModel,
    updateClassicModel
} from "../../../http/directorysApi";


const ClassicEdit =  ({show, onHide}) => {

    const {classicModels} = useContext(Context);
    const [cModel,     setCModel]      = useState('')


    // При открытии страницы
    const SetParams = () => {
        if (classicModels.isNew) {  setCModel('')
        } else {   setCModel(classicModels.getSelectedOne().model);        }}

    const createModel = () => {
        createClassicModel({ model: cModel}).then(data => {
            // После создания клиента запрашиваем обновленный список и выделяем клиента которого создали
            getClassicModel().then(data => {
                classicModels.setData(data)
                if (classicModels.Data.length > 0) {
                    classicModels.setSelectedOne(classicModels.Data[classicModels.Data.length-1])
                }
            })
            onHide()
        })
    }

    const updateModel = () => {
        updateClassicModel({model : cModel, id:classicModels.getSelectedOne().id} ).then(data => {
            // После создания клиента запрашиваем обновленный список и выделяем клиента которого создали

            onHide()
            getClassicModel().then(data => {
                classicModels.setData(data)
            } )
        })
    }

    return (
        <Modal show={show}  onHide={onHide} size="lg" centered  onShow={SetParams} >
            <Modal.Header closeButton>
                <Modal.Title id="">
                    {classicModels.isNew? 'Добавить модель' : 'Изменить модель'}
                </Modal.Title>
            </Modal.Header>
            <Modal.Body>
                <form>
                    <div className="input-group input-group-sm mb-3">
                        <span className="input-group-text col-2" id="inputGroup-sizing-sm">Название</span>
                        <input type="text" className="form-control" value={cModel}  onChange={e=>setCModel(e.target.value)}   ></input>
                    </div>
                </form>

            </Modal.Body>


            <Modal.Footer>
                <Button className="m-1 btn-sm btn btn-secondary" onClick={onHide}>Закрыть</Button>
                <Button className="m-1 btn-sm btn btn-secondary" onClick={ classicModels.isNew? createModel : updateModel}>{classicModels.isNew? 'Добавить модель' : 'Изменить модель'}</Button>
            </Modal.Footer>

        </Modal>
    );
};

export default ClassicEdit;